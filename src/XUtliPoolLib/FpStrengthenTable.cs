...
namespace XUtliPoolLib
{
	public class FpStrengthenTable : CVSReader
	{
		...
		public List<FpStrengthenTable.RowData> Table = new List<FpStrengthenTable.RowData>();
		...
		private FpStrengthenTable.RowData BinarySearchBQID(int key, int min, int max)
		{
			FpStrengthenTable.RowData rowData = this.Table[min];
			if (rowData.BQID == key)
			{
				return rowData;
			}
			FpStrengthenTable.RowData rowData2 = this.Table[max];
			...
			FpStrengthenTable.RowData rowData3 = this.Table[num];
		...
		private void AddRowBQID(int key, FpStrengthenTable.RowData row, int min, int max)
		{
			if (this.Table.Count == 0)
			{
				this.Table.Add(row);
			...
			if (rowData.BQID == key)
			{
				XSingleton<XDebug>.singleton.AddErrorLog2("Table: FpStrengthenTable duplicate id:{0}  lineno: {1}", new object[]
				{
					key,
					this.lineno
				});
