...
public class UILabel : UIWidget
{
	...
	public void UpdateDefaultPrintedSize()
	{
		this.mPrintedSize = Mathf.Abs(this.defaultFontSize);
	}
		...
		if (this.mPrintedSize > 0)
		{
			...
			for (int i = this.mPrintedSize; i > 0; i--)
			{
				...
				if (this.mOverflow != UILabel.Overflow.ShrinkContent || flag2)
				{
					if (this.mOverflow == UILabel.Overflow.ResizeFreely)
					{
						this.mCalculatedSize = NGUIText.CalculatePrintedSize(this.mProcessedText, -1);
	...
	public override void OnFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		if (this.mOutLineLock && this.effectStyle != UILabel.Effect.None)
		{
