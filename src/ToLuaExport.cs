...
public static class ToLuaExport
{
	...
	{
		ToLuaExport.<Generate>c__AnonStorey74 <Generate>c__AnonStorey = new ToLuaExport.<Generate>c__AnonStorey74();
		...
		{
			int num = list.FindIndex((MethodInfo m) => m.Name == "get_" + <Generate>c__AnonStorey.ps[i].Name);
			if (num >= 0 && list[num].Name != "get_Item")
			{
				list.RemoveAt(num);
			}
			num = list.FindIndex((MethodInfo m) => m.Name == "set_" + <Generate>c__AnonStorey.ps[i].Name);
	...
	private static void GenFunction()
	{
		...
		for (int i = 0; i < ToLuaExport.methods.Length; i++)
		{
			...
			if (methodInfo.IsGenericMethod)
			{
				Debugger.Log("Generic Method {0} cannot be export to lua", new object[]
				{
					methodInfo.Name
				});
			}
	...
	{
		ToLuaExport.<GenConstruct>c__AnonStorey77 <GenConstruct>c__AnonStorey = new ToLuaExport.<GenConstruct>c__AnonStorey77();
		...
		<GenConstruct>c__AnonStorey.list = new List<ConstructorInfo>();
		for (int k = 0; k < constructors.Length; k++)
		{
			if (!ToLuaExport.HasDecimal(constructors[k].GetParameters()))
			{
				if (!ToLuaExport.IsObsolete(constructors[k]))
				{
					...
					int num = <GenConstruct>c__AnonStorey.list.FindIndex((ConstructorInfo p) => ToLuaExport.CompareMethod(p, r) >= 0);
					if (num >= 0)
					{
						if (ToLuaExport.CompareMethod(<GenConstruct>c__AnonStorey.list[num], r) == 2)
						{
							<GenConstruct>c__AnonStorey.list.RemoveAt(num);
							<GenConstruct>c__AnonStorey.list.Add(r);
						}
					}
					else
					{
		...
		if (<GenConstruct>c__AnonStorey.list.Count == 0)
		{
			if (!ToLuaExport.type.IsValueType)
			{
				ToLuaExport.NoConsturct();
		...
		<GenConstruct>c__AnonStorey.list.Sort(new Comparison<ConstructorInfo>(ToLuaExport.Compare));
		ToLuaExport.sb.AppendLine("\r\n\t[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]");
		ToLuaExport.sb.AppendFormat("\tstatic int _Create{0}(IntPtr L)\r\n", ToLuaExport.wrapClassName);
		ToLuaExport.sb.AppendLine("\t{");
		ToLuaExport.sb.AppendLine("\t\tint count = LuaDLL.lua_gettop(L);");
		...
		int i;
		for (i = 0; i < <GenConstruct>c__AnonStorey.list.Count; i++)
		{
			int num2 = <GenConstruct>c__AnonStorey.list.FindIndex((ConstructorInfo p) => p != <GenConstruct>c__AnonStorey.list[i] && p.GetParameters().Length == <GenConstruct>c__AnonStorey.list[i].GetParameters().Length);
			if (num2 >= 0 || (ToLuaExport.HasOptionalParam(<GenConstruct>c__AnonStorey.list[i].GetParameters()) && <GenConstruct>c__AnonStorey.list[i].GetParameters().Length > 1))
			{
				list.Add(<GenConstruct>c__AnonStorey.list[i]);
			}
		}
		MethodBase methodBase = <GenConstruct>c__AnonStorey.list[0];
		bool flag = <GenConstruct>c__AnonStorey.list[0].GetParameters().Length == 0;
		...
		else
		{
			...
			if (<GenConstruct>c__AnonStorey.list.Count == 1 || methodBase.GetParameters().Length != <GenConstruct>c__AnonStorey.list[1].GetParameters().Length)
			{
		...
		ToLuaExport.sb.AppendLine("\t\t{");
		int num3 = ToLuaExport.ProcessParams(methodBase, 3, true, <GenConstruct>c__AnonStorey.list.Count > 1, false);
		ToLuaExport.sb.AppendFormat("\t\t\treturn {0};\r\n", num3);
		ToLuaExport.sb.AppendLine("\t\t}");
		for (int j = 1; j < <GenConstruct>c__AnonStorey.list.Count; j++)
		{
			flag = (<GenConstruct>c__AnonStorey.list[j].GetParameters().Length == 0 || flag);
			methodBase = <GenConstruct>c__AnonStorey.list[j];
			ParameterInfo[] parameters3 = methodBase.GetParameters();
			if (!ToLuaExport.HasOptionalParam(methodBase.GetParameters()))
			{
				if (list.Contains(<GenConstruct>c__AnonStorey.list[j]))
				{
	...
	{
		ToLuaExport.<GenOverrideFunc>c__AnonStorey7A <GenOverrideFunc>c__AnonStorey7A = new ToLuaExport.<GenOverrideFunc>c__AnonStorey7A();
		<GenOverrideFunc>c__AnonStorey7A.list = new List<MethodInfo>();
		for (int k = 0; k < ToLuaExport.methods.Length; k++)
		{
			...
			{
				ToLuaExport.Push(<GenOverrideFunc>c__AnonStorey7A.list, ToLuaExport.methods[k]);
			}
		}
		if (<GenOverrideFunc>c__AnonStorey7A.list.Count == 1)
		{
			return <GenOverrideFunc>c__AnonStorey7A.list[0];
		}
		<GenOverrideFunc>c__AnonStorey7A.list.Sort(new Comparison<MethodInfo>(ToLuaExport.Compare));
		ToLuaExport.sb.AppendLine("\r\n\t[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]");
		...
		List<MethodInfo> list = new List<MethodInfo>();
		int i;
		for (i = 0; i < <GenOverrideFunc>c__AnonStorey7A.list.Count; i++)
		{
			int num = <GenOverrideFunc>c__AnonStorey7A.list.FindIndex((MethodInfo p) => ToLuaExport.CompareParmsCount(p, <GenOverrideFunc>c__AnonStorey7A.list[i]));
			if (num >= 0 || (ToLuaExport.HasOptionalParam(<GenOverrideFunc>c__AnonStorey7A.list[i].GetParameters()) && <GenOverrideFunc>c__AnonStorey7A.list[i].GetParameters().Length > 1))
			{
				list.Add(<GenOverrideFunc>c__AnonStorey7A.list[i]);
			}
		}
		ToLuaExport.sb.AppendLine();
		MethodInfo methodInfo = <GenOverrideFunc>c__AnonStorey7A.list[0];
		...
		int num6 = <GenOverrideFunc>c__AnonStorey7A.list[1].GetParameters().Length + ((!<GenOverrideFunc>c__AnonStorey7A.list[1].IsStatic) ? 1 : 0);
		bool flag = true;
		...
		ToLuaExport.sb.AppendLine("\t\t{");
		int num7 = ToLuaExport.ProcessParams(methodInfo, 3, false, <GenOverrideFunc>c__AnonStorey7A.list.Count > 1 && flag, beCheckTypes);
		ToLuaExport.sb.AppendFormat("\t\t\treturn {0};\r\n", num2 + num7);
		ToLuaExport.sb.AppendLine("\t\t}");
		for (int j = 1; j < <GenOverrideFunc>c__AnonStorey7A.list.Count; j++)
		{
			flag = true;
			beCheckTypes = true;
			methodInfo = <GenOverrideFunc>c__AnonStorey7A.list[j];
			...
			if (!ToLuaExport.HasOptionalParam(methodInfo.GetParameters()))
			{
				...
				if (list.Contains(<GenOverrideFunc>c__AnonStorey7A.list[j]))
				{
	...
	private static void NewIndexSetValue(Type t, string o, string name)
	{
		...
		{
			Debugger.LogError("not defined type {0}", new object[]
			{
				t
			});
		}
	...
	private static void ProcessExtends(List<MethodInfo> list)
	{
		...
		if (ToLuaExport.extendType != null)
		{
			ToLuaExport.<ProcessExtends>c__AnonStorey7C <ProcessExtends>c__AnonStorey7C = new ToLuaExport.<ProcessExtends>c__AnonStorey7C();
			<ProcessExtends>c__AnonStorey7C.list2 = new List<MethodInfo>();
			<ProcessExtends>c__AnonStorey7C.list2.AddRange(ToLuaExport.extendType.GetMethods(BindingFlags.Instance | ToLuaExport.binding | BindingFlags.DeclaredOnly));
			int i;
			for (i = <ProcessExtends>c__AnonStorey7C.list2.Count - 1; i >= 0; i--)
			{
				if ((!<ProcessExtends>c__AnonStorey7C.list2[i].Name.Contains("op_") && !<ProcessExtends>c__AnonStorey7C.list2[i].Name.Contains("add_") && !<ProcessExtends>c__AnonStorey7C.list2[i].Name.Contains("remove_")) || ToLuaExport.IsNeedOp(<ProcessExtends>c__AnonStorey7C.list2[i].Name))
				{
					list.RemoveAll((MethodInfo md) => md.Name == <ProcessExtends>c__AnonStorey7C.list2[i].Name);
					if (!ToLuaExport.IsObsolete(<ProcessExtends>c__AnonStorey7C.list2[i]))
					{
						list.Add(<ProcessExtends>c__AnonStorey7C.list2[i]);
					}
				}
			}
		}
