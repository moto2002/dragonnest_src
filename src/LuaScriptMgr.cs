...
public class LuaScriptMgr
{
	...
	{
		global::Debugger.Log("SendGMmsg", new object[0]);
		...
		for (int i = 0; i < param.Length; i++)
		{
			...
			{
				text = text + " " + text2;
				global::Debugger.Log(text2, new object[0]);
			}
			num++;
		}
	...
	private void PrintLua(params string[] param)
	{
		if (param.Length != 2)
		{
			global::Debugger.Log("PrintLua [ModuleName]", new object[0]);
			return;
		}
	...
	public void Start()
	{
		...
		{
			global::Debugger.LogWarning("BaseType {0} not register to lua", new object[]
			{
				current.FullName
			});
		}
	...
	public void Destroy()
	{
		...
		global::Debugger.Log("Lua module destroy", new object[0]);
	}
	...
	public LuaFunction GetLuaFunction(string name)
	{
		...
		if (!this.dict.TryGetValue(name, out luaBase))
		{
			...
			{
				global::Debugger.LogError("Lua function {0} not exists", new object[]
				{
					name
				});
			}
	...
	public int GetFunctionRef(string name)
	{
		...
		{
			global::Debugger.LogWarning("Lua function {0} not exists", new object[]
			{
				name
			});
		}
	...
	public byte[] Loader(string name)
	{
		...
		if (name.StartsWith("Hotfix") || name.EndsWith("_pb") || name.EndsWith("_pb.lua") || name.StartsWith("Lua"))
		{
			TextAsset textAsset = XSingleton<XResourceLoaderMgr>.singleton.ReadLuaBytes(name);
			if (textAsset != null)
			{
				byte[] array = new byte[textAsset.bytes.Length];
				Array.Copy(textAsset.bytes, array, textAsset.bytes.Length);
				result = array;
				XSingleton<XResourceLoaderMgr>.singleton.ReleaseLuaBytes(textAsset);
			}
		}
		else
		{
			TextAsset textAsset2 = Resources.Load<TextAsset>(Util.LuaResourcePath(name));
			if (textAsset2 != null)
			{
				result = textAsset2.bytes;
			}
		}
		return result;
	}
	...
	private static bool PushLuaTable(IntPtr L, string fullPath)
	{
		...
		if (luaTypes != LuaTypes.LUA_TTABLE)
		{
			...
			global::Debugger.LogError("Push lua table {0} failed", new object[]
			{
				array[0]
			});
			return false;
	...
	public static void DumpStack(IntPtr L)
	{
		...
		while (i <= num)
		{
			...
			{
			case LuaTypes.LUA_TBOOLEAN:
				global::Debugger.Log(LuaDLL.lua_toboolean(L, i).ToString(), new object[0]);
				break;
			case LuaTypes.LUA_TLIGHTUSERDATA:
				goto IL_8C;
			case LuaTypes.LUA_TNUMBER:
				global::Debugger.Log(LuaDLL.lua_tonumber(L, i).ToString(), new object[0]);
				break;
			case LuaTypes.LUA_TSTRING:
				global::Debugger.Log(LuaDLL.lua_tostring(L, i), new object[0]);
				break;
			default:
				goto IL_8C;
			}
			...
			continue;
			IL_8C:
			global::Debugger.Log(luaTypes.ToString(), new object[0]);
			goto IL_A7;
		}
	}
