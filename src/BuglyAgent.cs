...
public sealed class BuglyAgent
{
	...
	private const int SDK_TYPE = 2;

	private const int SDK_LOG_LEVEL = 2;
	...
	{
		if (!BuglyAgent._debugMode && level != LogSeverity.Log && level < LogSeverity.LogWarning)
		{
			return;
		}
		try
