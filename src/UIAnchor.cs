...
public class UIAnchor : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		this.wc = ((!(this.container == null)) ? this.container.GetComponent<UIWidget>() : null);
		this.pc = ((!(this.container == null) || !(this.wc == null)) ? this.container.GetComponent<UIPanel>() : null);
	}
	...
	private void Update()
	{
		...
		if (this.wc != null)
		{
			Bounds bounds = this.wc.CalculateBounds(this.container.transform.parent);
		...
		{
			if (this.pc.clipping == UIDrawCall.Clipping.None)
			{
			...
			{
				Vector4 finalClipRegion = this.pc.finalClipRegion;
		...
		else
		{
			...
			if (this.pc != null)
			{
				vector = this.pc.cachedTransform.TransformPoint(vector);
			}
			else if (this.container != null)
			{
