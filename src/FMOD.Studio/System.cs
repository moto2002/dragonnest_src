...
namespace FMOD.Studio
{
	public class System : HandleBase
	{
		...
		public RESULT getBankByID(Guid guid, out Bank bank)
		{
			bank = null;
			IntPtr raw = 0;
			RESULT rESULT = System.FMOD_Studio_System_GetBankByID(this.rawPtr, guid.ToByteArray(), out raw);
			if (rESULT != RESULT.OK)
			{
				return rESULT;
			}
		...
		public RESULT lookupID(string path, out Guid guid)
		{
			...
			RESULT result = System.FMOD_Studio_System_LookupID(this.rawPtr, Encoding.UTF8.GetBytes(path + '\0'), array);
			guid = new Guid(array);
			return result;
		}
		...
		[DllImport("fmodstudio")]
		private static extern RESULT FMOD_Studio_System_LookupID(IntPtr studiosystem, byte[] path, byte[] guid);
