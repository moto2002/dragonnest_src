...
namespace com.tencent.pandora.MiniJSON
{
	public static class Json
	{
		private sealed class Parser : IDisposable
		{
			...
			{
				Json.Parser._reusedBuilder.Remove(0, Json.Parser._reusedBuilder.Length);
				this.json.Read();
				bool flag = true;
				while (flag)
				{
					...
					if (c != '"')
					{
						if (c != '\\')
						{
							Json.Parser._reusedBuilder.Append(nextChar);
						}
						else
						{
							if (this.json.Peek() != -1)
							{
								...
								{
								case 'n':
									Json.Parser._reusedBuilder.Append('\n');
									continue;
								case 'o':
								case 'p':
								case 'q':
								case 's':
									...
									if (c2 == 'b')
									{
										Json.Parser._reusedBuilder.Append('\b');
										continue;
									}
									if (c2 != 'f')
									{
									...
									Json.Parser._reusedBuilder.Append('\f');
									continue;
								case 'r':
									Json.Parser._reusedBuilder.Append('\r');
									continue;
								case 't':
									Json.Parser._reusedBuilder.Append('\t');
									continue;
								case 'u':
								{
									...
									Json.Parser._reusedBuilder.Append((char)Convert.ToInt32(new string(array), 16));
									continue;
								}
								}
								goto IL_B5;
				...
				return Json.Parser._reusedBuilder.ToString();
			}
