...
public class UIScrollView : MonoBehaviour
{
	...
	protected bool mAutoMove;

	protected float mMoveAmount;

	protected float mSrcAmount;

	protected float mDestAmount;

	protected float mMoveSpeed;

	public float moveControllerTime;
	...
	public bool RestrictWithinBounds(bool instant, bool horizontal, bool vertical)
	{
		...
		if (vector.sqrMagnitude > 1f)
		{
			if (!instant && this.dragEffect == UIScrollView.DragEffect.MomentumAndSpring)
			{
				...
				this.sp = SpringPanel.Begin(this.mPanel.gameObject, this.sp, pos, 13f);
			}
			else
			{
	...
	public void ResetPosition(float pos)
	{
		if (NGUITools.GetActive(this))
		{
			this.mCalculatedBounds = false;
			Vector2 pivotOffset = NGUIMath.GetPivotOffset(this.contentPivot);
			this.SetDragAmount(pivotOffset.x, pos, false);
			this.SetDragAmount(pivotOffset.x, pos, true);
		}
	}
	...
	public void Drag()
	{
		if (base.enabled && NGUITools.GetActive(base.gameObject) && this.mShouldMove && Time.time > this.moveControllerTime)
		{
			if (this.mDragID == -10)
			{
	...
	public void SetAutoMove(float from, float to, float moveSpeed)
	{
		...
		this.mMoveAmount = from;
		this.mSrcAmount = from;
		this.mDestAmount = to;
	}
}
